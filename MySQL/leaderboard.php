<?php
    ...
    $showrecord = mysql_real_escape_string($_GET['showrecord'], $db); 
 
     //This query grabs the top 10 scores, sorting by score and timestamp.
    $query = "SELECT * FROM Scores WHERE gameversion = '$gameversion' AND track = '$track' AND coursetype = '$coursetype' AND weather = '$weather' ORDER by score ASC, ts ASC LIMIT $showrecord";
    ...
    for($i = 0; $i < $result_length; $i++)
    {
         ...
         echo $i + 1 . "," . $row['name'] . "," . $row['score'] . "," . $row['car'] . "|"; // And output them
    }
?>